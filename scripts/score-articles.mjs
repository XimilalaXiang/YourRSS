...
function buildScoringPrompt(articles, lang) {
...
1. **relevance** (1-10): How relevant to tech/AI/engineering
...
async function main() {
  ...
  if (provider === 'agent') {
    const prompt = buildScoringPrompt(articles, language);
    const output = {
      mode: 'agent',
    ...
    if (articles.length <= batchSize) {
      ...
      for (let i = 0; i < articles.length; i += batchSize) {
        ...
        const prompt = buildScoringPrompt(batch, language);
